...
static void select_devices(struct audio_device *adev)
{
    ...
    ALOGV("***** %s: SOUND_DBG   adev_in_devices=%#x, adev_out_device = %#x, output_device_id=%#x",
          __func__, adev->in_device, adev->out_device, output_device_id);
	...
	//audio_route_apply_and_update_path(adev->ar, output_route);
	...
	//audio_route_apply_and_update_path(adev->ar, output_gain);
        ALOGV("%s: Applying gain [%s] for route [%s]", __func__,
              output_gain, output_route);
    }
    if (input_route != NULL) {
	//audio_route_apply_and_update_path(adev->ar, input_route);
        ...
        snprintf(input_gain,
                 ...
                 "gain-%s",
                 input_route);
	//audio_route_apply_and_update_path(adev->ar, input_gain);
...
static void start_comm_call(struct audio_device *adev)
{
    if (adev->in_comm_call) {
        return;
    }
    adev->in_comm_call = true;
    ...
    adev->input_source = AUDIO_SOURCE_VOICE_COMMUNICATION;
    select_devices(adev);
...
//                   device_type = SOUND_AUDIO_PATH_BLUETOOTH_NO_NR;
    ...
    ALOGV("%s: SOUND_DBG : need device - (%#x) ||||| use audiopath - (%#x)",
          __func__, adev->out_device, device_type);
...
static int start_output_stream(struct stream_out *out)
{
    ...
    ALOGV("%s: stream out device: %#x, actual: %#x",
                                   ...
                                   const char *address __unused)
{
    ...
    ALOGV("%s: SOUND_DBG enter: sample_rate(%d) channel_mask(%#x) devices(%#x) flags(%#x)",
